...
class Scene : RefCounted {
    ...
    this(Device device, RenderTargetView!Rgba8 winRtv, DepthStencilView!Depth winDsv) {
        ...
        auto shadowSampler = new Sampler(shadowSrv, 
                    ...
                    .withComparison(s.Comparison.LessEqual))
                .rc;
        ...
        auto makeMesh(mat4 modelMat, float[4] color, VertexBuffer!Vertex vbuf, VertexBufferSlice slice) {
            ShadowPipeline.Data shadowData;
            ...
            return Mesh(modelMat, vec4(color), shadowData, meshData, slice);
        }
        ...
        auto makeCube(float[3] pos, float scale, float angle, float[4] color) {
            ...
            return makeMesh(model, color, cubeBuf, cubeSlice);
        }
    
        

        this.camera = Camera(
            mat4.look_at(vec3(3, -10, 6), vec3(0, 0, 0), vec3(0, 0, 1)),
            mat4.perspective(100, 100, 45, near, far),
        );
        this.lights = [
        ...
        this.meshes = [
            ...
            makeMesh(mat4.identity, [1, 1, 1, 1], planeBuf, planeSlice)
        ];
    }
